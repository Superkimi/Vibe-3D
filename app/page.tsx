...
const capabilities = [
  { icon: ChatCircleDots, title: "说出形状，也说出修改", copy: "从完整概念到毫米级调整，AI 将意图变成可审阅的场景操作。" },
  { icon: SlidersHorizontal, title: "每个参数仍在你手里", copy: "树状目录、空间变换、PBR 材质和灯光全部可以人工精调。" },
  { icon: BracketsCurly, title: "代码就是模型", copy: "VibeScene JSON 是唯一事实源。对话、画布和参数始终双向同步。" },
  { icon: Export, title: "带走标准资产", copy: "导出 GLB、OBJ、STL、JSON 或透明 PNG，不把作品锁在平台里。" },
];
  ...
  return (
    <main className="landing">
      ...
      <section id="workflow" className="capability-section">
        <h2>一个场景，三种入口</h2>
        <p className="section-intro">在画布里拖动，在右侧输入数值，或直接告诉 AI。每次变化都落到同一份 schema。</p>
      ...
      <section id="schema" className="schema-story">
        <div className="schema-copy">
          <h2>不是一次性的 AI 网格。</h2>
          <p>Vibe 3D 让 AI 生成结构化的节点、变换与材质。每轮修改可撤销、可比较，也能继续人工编辑。</p>
